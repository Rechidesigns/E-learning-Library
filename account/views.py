...
from .serializers import UserRegistrationSerializer, AuthorRegistrationSerializer, AdminRegistrationSerializer, UserDetailSerializer, LoginSerializer, UserLogoutSerializer, ChangePasswordSerializer
from rest_framework_simplejwt.tokens import RefreshToken
...
class AuthorRegisterView(APIView):
    def post(self, request):
        ...
        serializer.is_valid(raise_exception=True)
        account = serializer.save()
        data['response'] = 'successfully registered a new author.'
        data['email'] = account.email
        data['name'] = account.name
...
class AllAuthorsView(ListAPIView):
